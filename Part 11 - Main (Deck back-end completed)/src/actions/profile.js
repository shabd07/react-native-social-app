...
import { MYAGE_CHANGED, POP_UP_CHANGED, NAME_CHANGED, GENDER_CHANGED, INFO_CHANGED, AGE_CHANGED, IMAGE_PATH, IMAGE_ERROR } from '../types';
...
export const MyAgeChange = myage => ({
    type: MYAGE_CHANGED,
    payload: myage,
});
...
export const OnImagePressed = () => (dispatch) => {
    ImagePicker.showImagePicker(options, (response) => {
        ...
        } else if (response.height < 740) { // Reduced amount slightly
            dispatch({
                type: IMAGE_ERROR,
                payload: 'Height too small',
            });
        } else if (response.width < 740) {
            dispatch({
                type: IMAGE_ERROR,
...
const UploadImage = (user, imageSource) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        let uploadBlob = null;
        const imageRef = firebase.storage().ref().child(`/dp/${user.uid}/`);
...
export const OnSubmitPressed = (myage, user, name, gender, info, age, imageSource, allValid) => {
    if (allValid === '#4169E1') {
        // dispatch({ type: SUBMIT_PROFILE });
        const { uid } = user
        firebase.database().ref(`/userkey/${uid}`)
            .set({uid: true})
            .then(() => {
                firebase.database().ref(`/userdata/${uid}/`)
                    .set({ name, gender, info, age, myage })
                    .then(() => {
                        // Add notification
